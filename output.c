...
enum action {
	AC_PASSTHRU = 0,	/* print as-is (default) */
	AC_IGNORE,		/* no action */
...
struct parsetab {
	...
	char		*pt_arg;
} parsetab[128] = { AC_PASSTHRU, NULL };


/* validate terminal type and initialize parse table */
struct caps {
	char	cp_cap[4];	/* capability name */
	char	*cp_rep;	/* xterm replacement */
} caps[] = {
	"bc", "\b",		/* ^H */
	"bl", "\a",		/* ^G */
	"cd", ANSI_CLEAR_BELOW,
...
{
	static char cbuf[2048];	/* extracted capability value */
	char *cp;

	cp = cbuf;		/* reuse cbuf on each call */
	if (!(cp = tgetstr(cap, &cp)) || *cp < '0' || *cp > '9')
		return cp;
	while (*cp >= '0' && *cp <= '9')
		cp++;
	if (*cp == '.')
...
#define ERR_RET(fmt, val)			\
	...
	/* string capabilities */
	for (c = 0; c < 32; c++)	/* initialize non-printables */
		parsetab[c].pt_action = AC_IGNORE;
	parsetab[127].pt_action = AC_IGNORE;

	for (lp = caps; lp->cp_cap[0]; lp++) {
		if (!(cp = get_strcap(lp->cp_cap)))
			continue;
		if (!lp->cp_rep)
		...
		parsetab[cp[0]].pt_arg = lp->cp_rep;
	}
	if (cp = get_strcap("cl")) {
		switch (strlen(cp)) {
